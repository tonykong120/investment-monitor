...
def technical(h):
    if len(h)<20: return {"trend":50,"volume":50,"position":50,"trigger":None,"support":None,"vr":1,"reasons":[],"warnings":["历史数据不足"]}
    ...
    return {"trend":clamp(trend),"volume":volume,"position":position,"trigger":trigger,"support":support,"vr":vr,"reasons":reasons,"warnings":warnings}
...
def analyze(q,include_flow=False):
    t=technical(enrich_today(history(q["bs_code"]),q)); liquidity=clamp(45+math.log10(max(q["amount"]/1e8,1))*28); chase=max(0,100-max(q["pct"],0)*12)
    ...
    flow_score=50
    if flow["available"]: flow_score=clamp(50+(16 if flow["main_yi"]>0 else -10)+(10 if flow["positive_2"]==2 else 0))
    score=t["trend"]*.28+t["volume"]*.18+t["position"]*.18+liquidity*.16+chase*.10+flow_score*.10
    trig=t["trigger"] if t["trigger"] else q["price"]*1.01; trig=min(max(trig,q["price"]*.998),q["price"]*1.035)
    support=t["support"] if t["support"] else trig*.975; stop=min(trig*.975,support); rr1=(trig*1.035-trig)/max(trig-stop,.01)
    ...
    status="可买触发" if score>=80 and q["price"]>=trig*.995 and q["pct"]<4.5 else ("重点观察" if score>=70 else "放弃")
    return {"code":q["code"],"name":q["name"],"price":round(q["price"],2),"pct":round(q["pct"],2),"amount_yi":round(q["amount"]/1e8,2),"quote_source":q["quote_source"],"score":round(score,1),"flow_score":round(flow_score),"trend_score":round(t["trend"]),"volume_score":round(t["volume"]),"position_score":round(t["position"]),"liquidity_score":round(liquidity),"volume_ratio":round(t["vr"],2),"trigger":round(trig,2),"stop":round(stop,2),"target1":round(trig*1.035,2),"target2":round(trig*1.06,2),"rr1":round(rr1,2),"status":status,"reasons":reasons[:5],"warnings":warnings[:4],"fund_flow":flow}
...
def main():
    ...
    try:
        ...
        data={"version":"1.3","updated_at":now,"market":{"score":market_score,"state":market_state,"up_ratio":round(up,1),"median_pct":round(med,2),"universe":len(quotes)},"opportunities":[x for x in candidates if x["status"]!="放弃"][:3],"watchlist":watch,"candidates":candidates[:20],"source_health":source_health,"rules":{"price":"5-30元","amount":"≥3亿元","day_pct":"-7.5%~+5%","excluded":"科创/创业/北交/ST/退市","holding":"1-5个交易日"}}
